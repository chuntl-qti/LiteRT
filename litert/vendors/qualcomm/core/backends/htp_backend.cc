...
HtpBackend::~HtpBackend() { perf_control_.Terminate(); }

HtpBackend::QnnDevicePlatformInfo HtpBackend::GetDevicePlatformInfo() {
...
bool HtpBackend::Init(const Options &options,
  ...
  QnnDevicePlatformInfo local_qnn_device_platform_info = {
      nullptr, PlatformInfoDeleter{QnnApi()}};
  ...
  if (soc_info.has_value()) {
    ...
    local_qnn_device_platform_info = GetDevicePlatformInfo();
    if (!local_backend_handle) {
      QNN_LOG_WARNING("Fail to get platforminfo: %d, using default.");
    } else {
